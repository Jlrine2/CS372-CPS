...
TEST_CASE("Spacer dimension calculations") {
    std::shared_ptr<Shape> s1 = std::make_shared<Spacer>(1.0, 2.0);

    REQUIRE(s1->get_width() == Approx(1.0));
    REQUIRE(s1->get_height() == Approx(2.0));

    std::shared_ptr<Shape> s2 = std::make_shared<Spacer>(2.0, 2.0);
